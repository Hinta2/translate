...
Features:
- Reads text from a user-selected Windows window (default: "Live captions") using UI Automation (pywinauto).
...
- Global hotkeys:
    ...
    Ctrl+Shift+S -> Select source window
- Starts listening automatically on launch.
- Lets you choose the source window at runtime.
...
DEFAULT_SOURCE_PATTERN = os.getenv("SOURCE_WINDOW_TITLE", LIVE_CAPTIONS_TITLE_PATTERN)
...
class LiveCaptionsReader:
    """Reads current text from a selected Windows window via UI Automation."""

    def __init__(self, title_pattern: str = DEFAULT_SOURCE_PATTERN):
    ...
    def set_title_pattern(self, title_pattern: str) -> None:
        with self._lock:
            self._title_pattern = title_pattern
...
class OverlayApp:
    def __init__(self):
        ...
        self.source_window_title = "Live captions (default)"
        ...
        self.menu.add_command(label="Select source window", command=self.select_source_window)
        ...
        self.root.bind_all("<F10>", lambda _e: self.select_source_window())
    ...
    def _escape_title_for_regex(self, title: str) -> str:
        return rf"^{re.escape(title)}$"

    def select_source_window(self):
        """
        Let user select the currently focused window as the capture source.
        """
        messagebox.showinfo(
            "Select source window",
            "1) Focus the target window that has live English text.\n"
            "2) Click OK, then quickly activate that window.",
        )
        time.sleep(1.2)
        ...
        if not title:
            manual = simpledialog.askstring(
                "Window title",
                "Couldn't detect active window title.\nEnter exact window title manually:",
            )
            title = (manual or "").strip()

        if not title:
            messagebox.showwarning("Selection failed", "No window title selected.")
            return

        self.source_window_title = title
        self.reader.set_title_pattern(self._escape_title_for_regex(title))
        self.clear_text()
        self.text_var.set(f"Source selected: {title}\nListening...")
            ...
            if full_text is None:
                ...
                if now - self.last_warning_time > 5:
                    self.ui_queue.put(
                        (
                            "warning",
                            f"Source window not detected: {self.source_window_title}\n"
                            "Use right-click -> Select source window.",
                        )
                    )
    ...
    def _register_global_hotkeys(self):
        ...
        _safe_register("ctrl+shift+s", self.select_source_window)
